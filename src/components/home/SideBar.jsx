...
// ✅ تم حذف الـ props القديمة: ({ currentUser: initialUser, onUserUpdate })
const SideBar = () => { 
    const navigate = useNavigate();
    const location = useLocation();
    // ✅ استيراد دالة التحديث من Context
    const { userProfileData, updateProfileImage, updateUserProfile } = useUser(); 

    // ✅ استخدام حالة Context كقيمة أولية
    ...
    const fetchLatestUserData = async () => {
        try {
            ...
            const updatedUser = {
                name: user.fullName || "User",
                // ✅ تغيير المفتاح إلى 'profileImage' لمطابقة Context
    ...
    return (
        ...
        >
            ...
            >
                <div className="card-body p-4 text-center">
                    <img
                        // ✅ استخدام الصورة من state المحلي الذي تم تحديثه من الـ API
                        src={currentUser?.profileImage || PLACEHOLDER_AVATAR} 
                        alt="Profile"
                        className="rounded-circle mb-3"
                        style={{ 
                            width: '80px', 
