...
// SideBar.jsx - UPDATED WITH CLOSE BUTTON
import React, { useEffect, useState } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { authAPI } from "../../services/api";
import "../../styles/Home/SideBar.css";

const SideBar = ({ currentUser: initialUser, onUserUpdate, onClose }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [currentUser, setCurrentUser] = useState(initialUser);
  const [refreshKey, setRefreshKey] = useState(0);
        ...
        <div className="card-body p-4 text-center">
          ...
          <div className="mb-3 p-3 rounded" style={{ backgroundColor: '#f0f9f7' }}>
            <div className="d-flex align-items-center justify-content-center gap-2">
              <i className="bi bi-trophy-fill text-warning" style={{ fontSize: '1.5rem' }}></i>
              <div>
                <h6 className="mb-0 fw-bold" style={{ color: '#386641' }}>
                  {currentUser?.ecoPoints || 0} Points
                </h6>
                <small className="text-muted">{currentUser?.level || "Eco Newbie"}</small>
              </div>
            ...
            {menuItems.map((item, index) => (
              <button
                ...
                style={{ 
                  ...
                  transition: 'all 0.2s',
                  border: 'none',
                  backgroundColor: isActive(item.path) ? '#386641' : '#f8f9fa',
                  color: isActive(item.path) ? 'white' : 'inherit'
                }}
                onMouseEnter={(e) => {
                  if (!isActive(item.path)) {
