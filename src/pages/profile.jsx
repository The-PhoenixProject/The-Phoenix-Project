// // profile.jsx - COMPLETELY FIXED VERSION WITH SESSION MANAGEMENT
...
//         // If token is cleared, redirect to login
...
//   // ✅ Session check and redirect function
//   const checkSessionAndRedirect = useCallback((error) => {
...
//   if (!token) {
//     checkSessionAndRedirect({ message: "No token found" });
//     return;
//   }
...
//       image: getImageUrl(u.profilePicture || u.avatar || u.image), 
...
//       // ✅ FIX: Use array lengths instead of count fields
...
//     throw err;
//   }
// }, [token, getImageUrl, checkSessionAndRedirect]);
...
//           const productsData = productsRes?.data?.products || productsRes?.products || productsRes?.data || [];
...
//     fetchProfileData();
//   }, [token, navigate, refreshUserData, checkSessionAndRedirect]);
...
//       });
//     } catch (error) { 
//       if (checkSessionAndRedirect(error)) return;
...
//       // Handle different response structures
//       const newPost = data?.post || data?.data?.post || data?.data || data;
...
export default function ProfilePage() {
    ...
    const fileInputRef = useRef(null);
    const token = localStorage.getItem("authToken");
    const PLACEHOLDER_IMAGE = "https://t3.ftcdn.net/jpg/11/61/33/36/360_F_1161333642_i694dqMuUwQEpEPdQOhuxdRC9WHPREFJ.jpg";
    const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000';
    ...
    const refreshUserData = useCallback(async () => {
        try {
            ...
            setUser({
                ...
                bio: u.bio || u.description || "Welcome to my eco-friendly profile! 🌿",
                image: profileImageUrl, // استخدام profileImageUrl المحدث
                location: u.location || "Not specified",
            ...
            // ✅ تحديث Context هنا أيضاً لضمان مزامنة بيانات الصورة عند تحميل الصفحة
    ...
    const deleteProfileImage = async () => {
        try {
            ...
            setUser(prev => ({ ...prev, image: PLACEHOLDER_IMAGE }));
            // ✅ تحديث Context بالصورة الافتراضية
